...
export function PostForm() {
  ...
  function construirPayload(): CrearPostCulturaPayload | null {
    ...
    switch (tipo) {
      case "foro":
        if (!categoria.trim())
          return (setFormError("Selecciona una categoría."), null);
  ...
  return (
    <form
      ...
      className="mx-auto max-w-xl px-4 py-6 md:px-8 md:py-8"
    >
      ...
      <div className="mt-6 grid grid-cols-2 gap-2 sm:flex">
        {TIPOS.map((t) => (
          <button
            ...
            className={`rounded-xl border px-3 py-2 text-sm font-medium transition sm:flex-1 ${
              tipo === t.value
